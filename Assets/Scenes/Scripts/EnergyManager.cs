...
public class EnergyManager : MonoBehaviour
{
	...
	void OnApplicationPause(bool pause)
	{
		...
		{
			// 🛑 BLOQUEO DE GUARDADO: 
			// Solo guarda el estado si el GameDataManager ha terminado su inicialización
			if (GameDataManager.IsReady)
			{
				GuardarEstado();
			}
			else
			{
				...
				Debug.LogWarning("Guardado en pausa ignorado: El sistema no está inicializado (IsReady = false).");
			}
		}
		else // pause = false (El juego se reanuda)
		{
			// 🛑 BLOQUEO DE REANUDACIÓN: 
			// Solo ejecuta la lógica de tiempo si el sistema está inicializado.
